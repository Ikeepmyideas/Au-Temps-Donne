...
<section class="content">
    ...
    ?>
            ...
            <a href="list_beneficiaire.php" class="<?php echo ($current_page == 'list_beneficiaire.php') ? 'active' : ''; ?>" style="<?php echo ($current_page == 'list_beneficiaire.php') ? 'color: #D23939;' : ''; ?>">Bénéficiaires</a>
...
fetch('http://localhost:8000/api/admin/beneficiaires')
    ...
    .then(data => {
        console.log('Données des bénéficiaires récupérées:', data);
        const tableBody = document.querySelector('table tbody');
        tableBody.innerHTML = ''; 
        
        data.forEach(beneficiaire => {
            const dateNaissance = new Date(beneficiaire.dateDeNaissance); 
    ...
    .catch(error => console.error('Erreur lors de la récupération des données des bénéficiaires:', error));

    function confirmerSuppression(email) {
    if (confirm('Êtes-vous sûr de vouloir supprimer ce bénévole ?')) {
        fetch('http://localhost:8000/api/admin/beneficiaires/delete', {
            method: 'DELETE',
            headers: {
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error('Erreur lors de la suppression du beneficiaire');
            }
            return response.json();
        })
        .then(data => {
            console.log('beneficiaire supprimé avec succès:', data);
            window.location.reload(); 
        })
        .catch(error => {
            console.error('Erreur lors de la suppression:', error);
            alert('Erreur lors de la suppression du beneficiaire.');
        });
    }
}
